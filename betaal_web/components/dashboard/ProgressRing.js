  ...
  return (
    ...
    >
      <div ref={ref} className="relative flex items-center justify-center p-8">
        <svg className="h-64 w-64 -rotate-90">
          ...
          <circle
            ...
            strokeDasharray={c}
            style={{ strokeDashoffset: off, transition: 'stroke-dashoffset 2s cubic-bezier(0.16, 1, 0.3, 1)', strokeLinecap: 'butt' }}
            fill="transparent"
          />
        </svg>
        <div className="absolute flex flex-col items-center">
          <span className="text-6xl font-black tracking-tighter text-[#1C1C1C] transition-colors duration-500" style={{ color: strokeColor }}>
            {Math.round(p)}%
          </span>
          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#1C1C1C]/30 mt-2">
            Consumed
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-1 mt-4">
        <span className="text-[11px] font-black uppercase tracking-[0.15em] text-[#1C1C1C]/40">
          {usage}m used of {quota}m
        </span>
        <p className="text-[10px] font-bold text-[#1C1C1C]/20 uppercase tracking-widest mt-1">
